...
/**
 * Get the element "value" to be serialized to as an attribute or a mutation record. It respects the
 * InputPrivacyMode of the input. An 'ancestorInputPrivacyMode' can be provided (if known) to
 * avoid iterating over the element ancestors when looking for the input privacy mode.
 */
export function getElementInputValue(element: Element, ancestorInputPrivacyMode?: InputPrivacyMode) {
  ...
  if (type === 'button' || type === 'submit' || type === 'reset') {
    // Always use the button and submit value, as they are used during replay to display the button
    // label. They can still be hidden via the "hidden" privacy attribute or class name.
    return value
  }
